...
interface Props {
  status: ScrapeStatus;
  onStart: (keywords: string, location: string, limit: number) => void;
  onStop: () => void;
}
...
export const ScraperForm: React.FC<Props> = ({ status, onStart, onStop }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLoading) {
      ...
      onStart(keywords, location, limit);
    }
  };
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end">
        ...
        <div className="w-full md:w-32">
             ...
             <select 
                ...
                className="block w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm bg-white"
             >
        ...
        <button 
            ...
            className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg font-medium text-white transition-all shadow-md active:scale-95 whitespace-nowrap ${
                isLoading 
      ...
      {isLoading && (
        <div className="mt-6">
            <div className="flex justify-between text-xs text-slate-500 mb-1">
                ...
                <span>Processing business listings...</span>
            </div>
